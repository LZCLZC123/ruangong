...
class Events {
    ...
    */
    public static function onMessage($client_id, $message) {
        echo "client={$_SERVER['REMOTE_ADDR']}:{$_SERVER['REMOTE_PORT']} client_id=$client_id gateway={$_SERVER['GATEWAY_ADDR']}:{$_SERVER['GATEWAY_PORT']} message=$message\n";
        $message_data = json_decode($message, true);
        if (!$message_data) {
            return;
        }
        if (isset($message_data['message']) && strlen($message_data['message']) >= 2048) {
            return;
        }
        switch ($message_data['type']) {
            ...
            case 'login':
                ...
                if (!empty($member)) {
                    self::$db->update(self::table('superman_hand2_member_login'))->cols(array(
                        'is_online' => 1
                    ))->where("id='{$member['id']}'")->query();
                } else {
                    self::$db->insert(self::table('superman_hand2_member_login'))->cols(array(
                        'uniacid' => $uniacid,
                        'uid' => $uid,
                        'is_online' => 1
                    ))->query();
            ...
            case 'say':
                ...
                $itemid = intval($message_data['item_id']);
                $sendtime = strtotime($message_data['createtime']);
                if (empty($itemid)) {
                    echo "itemid is null \n";
                    return;
                }
                ...
                if (!empty($last)) {
                    self::$db->update(self::table('superman_hand2_message_list'))->cols(array(
                        'status' => 1,
                    ...
                    self::$db->insert(self::table('superman_hand2_message_list'))->cols(array(
                    ...
                    $row = self::$db->select('*')->from(self::table('superman_hand2_message_list'))
                        ->where("uid='$from_uid' AND from_uid='$to_uid' AND itemid='$itemid'")->row();
                    if (empty($row)) {
                        self::$db->insert(self::table('superman_hand2_message_list'))->cols(array(
                            'uniacid' => $item['uniacid'],
                            'uid' => $from_uid,
                ...
                //记录消息
                $message_id = self::$db->insert(self::table('superman_hand2_message'))->cols(array(
                    'uniacid' => $item['uniacid'],
                    'itemid' => $itemid,
                    'from_uid' => $from_uid,
                    'to_uid' => $to_uid,
                ...
                if (Gateway::isUidOnline($to_uid)) {
                    //TODO: 判断当前页面，是否获取当前未读消息总数
                    /*$row = self::$db->select('COUNT(*)')->from(self::table('superman_hand2_message'))->where("uniacid='{$item['uniacid']}' AND uid='$to_uid' AND status=1")->row();
                    ...
                    Gateway::sendToUid($to_uid, json_encode(array(
                        ...
                        'message' => htmlspecialchars($message_data['message']),
                        'createtime' => $message_data['createtime'],
                    )));
                } else {
                    //TODO: 发送模板消息
                }
    ...
    */
    ...
    private static function getAccessToken($uniacid, $from) {
        if (empty($uniacid)) {
        ...
        $account = self::getWechatAccount($uniacid, $from);
        if (empty($account)) {
    ...
    private static function getWechatAccount($uniacid, $from) {
        if (empty($uniacid)) {
            return null;
        }
