...
if ($act == 'apply') {
    ...
    if (empty($itemid)) {
        SupermanHandUtil::json(SupermanHandErrno::INVALID_REQUEST, '物品id非法');
    }
    if (empty($total)) {
        SupermanHandUtil::json(SupermanHandErrno::INVALID_REQUEST, '请输入数量');
    }
    if ($paytype == 1) {
