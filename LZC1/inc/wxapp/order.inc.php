...
if ($act == 'display') {
    ...
    if (!$order) {
        SupermanHandUtil::json(SupermanHandErrno::INVALID_REQUEST, '订单不存在');
    }
    $item = pdo_get('superman_hand2_item', array(
        'uniacid' => $_W['uniacid'],
        'id' => $order['itemid'],
    ...
    if ($status == 3) {
        if ($order['credit'] > 0) {
            ...
            if (is_error($ret2)) {
                ...
                SupermanHandUtil::json(SupermanHandErrno::INVALID_REQUEST, '扣除积分失败');
            }
            //删除冻结积分
    ...
    $uid = intval($_GPC['uid']);
    if (!$uid) {
        SupermanHandUtil::json(SupermanHandErrno::DELETE_FAIL, '删除失败');
    }
    ...
    if ($list) {
        foreach ($list as &$row) {
            if ($row['uid'] > 0) {
                $user = pdo_get('mc_members', array('uid' => $row['uid']), array('nickname', 'avatar'));
    ...
    if (!$express_userid && !$express_apikey) {
        SupermanHandUtil::json(SupermanHandErrno::PARAM_ERROR, '未设置物流查询参数，请联系管理员');
    }
