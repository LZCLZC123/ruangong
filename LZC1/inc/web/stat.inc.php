...
if ($act == 'display') {
    ...
    $st = $_GPC['datelimit']['start'] ? $_GPC['datelimit']['start'] : date("Y-m-d H:i:s");
    $et = $_GPC['datelimit']['end'] ? $_GPC['datelimit']['end'] : date("Y-m-d H:i:s");
    $starttime = min($st, $et);
    $endtime = max($st, $et);
    $list = superman_hand2_stat(strtotime($starttime), strtotime($endtime));
    if ($_W['isajax']) {
...
function superman_hand2_stat($starttime, $endtime) {
    ...
    if ($starttime != $endtime) {
        $filter['daytime >='] = date('Ymd', $starttime);
        $filter['daytime <='] = date('Ymd', $endtime);
    }
