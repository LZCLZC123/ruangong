...
if ($act == 'display') {
    ...
    if (checksubmit('batch_submit')) {
        ...
        foreach ($itemids as $itemid) {
            if ($batch_operate == '批量审核通过') {
                ...
                if (!empty($uni_tpl_id) && !empty($gzh_appid)) {
                    $message_data['remark'] = array(
                        'value' => '点击进入小程序查看',
                        'color' => '#173177'
                    );
                    SupermanHandUtil::send_uniform_msg($message_data, $openid, $uni_tpl_id, $gzh_appid, $url);
                } else if (!empty($uni_tpl_id) && empty($gzh_appid)) {
                    $message_data['remark'] = array(
                        'value' => '如有疑问请联系客服',
                        'color' => '#173177'
                    );
                    $url = $_W['siteroot'].'app/'.$this->createMobileUrl('item', array('act' => 'detail', 'id' => $itemid));
                    SupermanHandUtil::send_tmplmsg($message_data, $openid, $uni_tpl_id, $url);
                }
            } else {
                $item = pdo_get('superman_hand2_item', array('id' => $itemid));
                $ret = pdo_update('superman_hand2_item', array(
    ...
    if ($list) {
        foreach ($list as &$li) {
            ...
            $li['order'] = pdo_get('superman_hand2_order', array('uniacid' => $_W['uniacid'], 'itemid' => $li['id']), array('createtime'));
    ...
    if (checksubmit()) {
        ...
        if (in_array($status, array('-3', '1'))) {
            ...
            if (!empty($uni_tpl_id) && !empty($gzh_appid)) {
                ...
                $url = $_W['siteroot'].'app/'.$this->createMobileUrl('item', array('act' => 'detail', 'id' => $id));
                SupermanHandUtil::send_tmplmsg($message_data, $openid, $uni_tpl_id, $url);
            }
        }
    ...
    if (checksubmit()) {
        ...
        $data = array(
            ...
            'audittime' => TIMESTAMP,
            'paytime' => TIMESTAMP,
            'expiretime' => strtotime("+{$total}day"),
        );
